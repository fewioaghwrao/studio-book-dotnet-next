...
export default function AdminStatusPage() {
  ...
  const fetchStatus = async (
    ...
    try {
      ...
      if (!response.ok) {
        setErrorMessage("データ一覧の取得に失敗しました。");
        return;
      }

      const json = (await response.json()) as AdminStatusResponse;
      setData(json);
    } catch {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:py-10">
      <nav className="mb-5 text-sm text-slate-500">
        ...
        <span className="text-slate-700">データ一覧</span>
      </nav>
      ...
      <section className="overflow-hidden rounded-[28px] border border-stone-200 bg-white shadow-sm">
        <div className="border-b border-stone-100 bg-gradient-to-br from-sky-50 via-white to-stone-50 px-5 py-7 text-center sm:px-8 sm:py-9">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            全スタジオを対象に、売上・稼働率・レビュー平均を確認できます。
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-8">
              <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <SummaryCard
                  label="登録スタジオ数"
                  value={`${formatNumber(data.totalRoomCount)}件`}
                />
                <SummaryCard
                  label="ホスト数"
                  value={`${formatNumber(data.totalHostCount)}人`}
                />
                <SummaryCard
                  label="予約件数"
                  value={`${formatNumber(data.totalReservationCount)}件`}
                />
<SummaryCard
  label="確定手数料売上合計"
  value={formatYen(data.totalPaidAmount)}
/>
              </section>
...
<p className="mt-1 text-sm text-slate-500">
  予約済みを見込み手数料売上、利用済みを確定手数料売上として表示します。
</p>
                </div>
